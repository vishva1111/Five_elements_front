    ...
    <Layout title="Task Management" subtitle="One task per tree — assign to Admin or Partner accounts">
      <div style={{ padding: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 24 }}>
          {['assigned', 'in_progress', 'completed', 'approved', 'rejected'].map(s => (
            <div key={s} style={{
              ...
              <div style={{ fontSize: 22, fontWeight: 800, color: STATUS_COLORS[s] }}>{statusCounts[s] || 0}</div>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
          ...
          <button onClick={loadTasks} style={btnSecondary}>↻ Refresh</button>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, minWidth: 240 }}>
                          <div style={{ display: 'flex', gap: 6 }}>
                            <input
                              ...
                              style={{ ...selectStyle, flex: 1, padding: '4px 8px', fontSize: 12 }}
                            />
                          </div>
                          <div style={{ display: 'flex', gap: 6 }}>
                            <button
                              ...
                              style={{ background: '#22c55e', color: '#fff', border: 'none', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: 11.5, fontWeight: 700, flex: 1 }}
                            >
                              {reviewing && reviewAction === 'approve' ? '…' : '✓ Approve'}
                            </button>
                            <button
                              disabled={reviewing}
                              onClick={() => handleReview(task.id, 'reject')}
                              style={{ background: '#ef4444', color: '#fff', border: 'none', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: 11.5, fontWeight: 700, flex: 1 }}
                            >
                              {reviewing && reviewAction === 'reject' ? '…' : '✕ Reject'}
                            </button>
                            <button
